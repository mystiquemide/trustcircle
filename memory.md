...
- May 2, 2026 (continued): PUBLIC CIRCLES DEPLOYMENT FIX - Diagnosed Vercel 404s on `/api/circles` as missing deployed backend API. Added Firebase HTTPS function `backend` serving `/api/circles`, `/api/invites`, and `/api/users`; deployed at `https://us-central1-trust-circle2.cloudfunctions.net/backend`. Updated frontend production API fallback to that URL. Fixed Public Circles visibility to hide only Resolved/Dissolved circles instead of all non-Pending circles. Added pending metadata recovery so CreateCircle stores public metadata locally and CircleDetail saves it once Arc resolves the contract address. Verified backend returns public metadata (`data: Array(1)`) and Public Circles displays the new circle after Vercel redeploy.
- May 2, 2026 (continued): PUBLIC LISTING REMOVAL - Added organizer-only "Remove from Public Circles" action on Pending public circles. This does not delete the onchain circle contract; it sets backend metadata `isPublic=false` so uninterested/stale circles disappear from public discovery while remaining accessible by direct link/address.
